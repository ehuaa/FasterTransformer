...
class EncoderWeights(object):
    ...
    def listed_weights(self):
        ...
        end_layer = (self.pipeline_para_rank + 1) * self.layer_num // self.pipeline_para_size
        if not self.int8:
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'attention.self.query.weight'].transpose(-1, -2)
            ...
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' +
                       'attention.self.query.bias'] for layer_idx in range(start_layer, end_layer)], 0).contiguous())
            ret[-1] = ret[-1].split(ret[-1].shape[-1] // self.tensor_para_size,
                                    dim=-1)[self.tensor_para_rank].contiguous()
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'attention.self.key.weight'].transpose(-1, -2)
            ...
            ret[-1] = ret[-1].split(ret[-1].shape[-1] // self.tensor_para_size,
                                    dim=-1)[self.tensor_para_rank].contiguous()
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'attention.self.value.weight'].transpose(-1, -2)
            ...
            ret[-1] = ret[-1].split(ret[-1].shape[-1] // self.tensor_para_size,
                                    dim=-1)[self.tensor_para_rank].contiguous()
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'attention.output.dense.weight'].transpose(-1, -2)
            ...
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'intermediate.dense.weight'].transpose(-1, -2)
            ...
            ret[-1] = ret[-1].split(ret[-1].shape[-1] // self.tensor_para_size,
                                    dim=-1)[self.tensor_para_rank].contiguous()
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'output.dense.weight'].transpose(-1, -2)
            ...
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'attention.self.key.weight']
            ...
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'intermediate.dense.weight']
            ...
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'output.dense.weight']
            ...
            ret.append(torch.stack([self.weights['roberta.encoder.layer.' + str(layer_idx) + '.' + 'amaxList']
